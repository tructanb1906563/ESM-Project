...
<template>
    ...
    <div class="space-y-10 flex flex-col items-center log">
        ...
        <div
            class="form flex-col flex items-center space-y-5 text-t-secondary font-bold"
        >
            <div class="space-y-3">
...
export default {
    ...
    methods: {
        async login() {
            ...
            const res = await axios.post('/auth/login', user).then(
                (res) => {
                    if (res.status == 201) {
                        this.$notify({
                            type: 'error',
        ...
        processUserInfo() {
            ...
            this.$notify({
                type: 'error',
                title: 'Error!',
                text: 'Please make sure to fill up the forms!',
            })
        },
    },
}
