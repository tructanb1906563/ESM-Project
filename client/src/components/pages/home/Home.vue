<template>
    <div class="flex justify-between">
        ...
        <button
            @click="notOpen"
            :class="{
                'bg-green-700/70 hover:bg-green-700': !isPostOpen,
                'bg-red-700/70 hover:bg-red-700/90 ': isPostOpen,
            }"
    ...
    <div
        v-if="!isFeedLoaded"
        class="text-red-500 opacity-75 flex justify-center mt-56"
    >
...
export default {
    ...
    watch: {
        isUserLoaded: async function (newer, old) {
            if (newer) await this.getFeed()
        },
    },
    async created() {
        if (this.isUserLoaded) await this.getFeed()
    },
    components: {
        Post,
        NewPost,
